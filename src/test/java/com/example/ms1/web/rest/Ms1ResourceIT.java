...
import com.example.ms1.config.KafkaProperties;
...
class Ms1ResourceIT {
    ...
    void producesMessages() throws Exception {
        restMockMvc.perform(post("/api/ms1-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc.perform(get("/api/ms1-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
